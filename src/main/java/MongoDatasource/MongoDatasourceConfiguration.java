...
public class MongoDatasourceConfiguration {
	...
	public static MongoDatasourceConfiguration loadFromJNDIReference(
		...
		// Optional DB port
		Integer optionalIntValue = getOptionalReferenceValueAsInteger(
				reference, PROP_PORT);
		...
		if (isEmptyValue(config.databaseName)) {
			throw new Exception("Missing value for mandatory property "
					+ PROP_DB_NAME);
		}
		// Optional user name
		...
		optionalIntValue = getOptionalReferenceValueAsInteger(reference,
				PROP_MIN_POOL_SIZE);
		if (optionalIntValue != null)
	...
	public MongoClientOptions getMongoClientOptions(final String datasourceName) {
		final MongoClientOptions.Builder optionBuilder = MongoClientOptions
				.builder();
	...
	private static Integer getOptionalReferenceValueAsInteger(
			final Reference reference, final String key) throws Exception {
		...
		if (!isEmptyValue(stringValue)) {
			try {
				...
				throw new Exception("Invalid " + key + " value " + stringValue,
						e);
			}
		}
		return null;
	...
	public String toString() {
		...
		// Authentication settings
		output.append(PROP_DB_NAME).append("=").append(this.databaseName)
				.append(", ");
		output.append(PROP_USERNAME).append("=").append(this.username)
				.append(", ");
		// Pooling settings
		output.append(PROP_MIN_POOL_SIZE).append("=").append(this.minPoolSize)
				.append(", ");
		output.append(PROP_MAX_POOL_SIZE).append("=").append(this.maxPoolSize)
				.append(", ");
